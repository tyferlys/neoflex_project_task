...
from includes.constant import metadata_dag_currency as metadata
from includes.utils.transform_date_to_db import transform_data_to_db


@logging_dag
def extract_file(**kwargs):
    df: pandas.DataFrame = pd.read_csv(metadata["directory_to_file"], sep=";", dtype=metadata["dtype"], encoding_errors="ignore")
...
with DAG(
        "etl_md_currency",
        default_args={},
        description="ETL process for csv file md_currency",
