...
def load_data_to_db(df: pd.DataFrame, metadata: dict, is_append, connection: str = "project-neoflex-db"):
    postgres_hook: PostgresHook = PostgresHook(connection)
    ...
    with engine.connect() as connection:
        if not is_append:
            ...
            df.to_sql(name=metadata["tablename"], schema=metadata["schema"], con=engine, if_exists="append",
                      index=False)
            return
